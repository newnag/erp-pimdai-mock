...
function getStatusText(status) {
    const statusMap = {
        'invoice-paid': 'เก็บเงินแล้ว',
        'invoice-partial': 'เก็บเงินบางส่วน',
        'invoice-pending': 'รอชำระเงิน',
...
function showContextMenu(event, docNo) {
    ...
    menu.innerHTML = `
        ...
        <div class="menu-item" data-action="payment">
...
function handleMenuAction(action, docNo) {
    switch(action) {
        case 'view':
            window.location.href = `invoice.html?doc=${docNo}`;
            break;
        case 'edit':
            window.location.href = `invoice.html?doc=${docNo}&mode=edit`;
            break;
        case 'payment':
            showNotification(`เปิดหน้าบันทึกการชำระเงินสำหรับ ${docNo}...`);
            break;
        case 'duplicate':
...
// Add context menu and info icon styles
...
style.textContent = `
    ...
    .status-dot.invoice-paid {
        background: #10b981;
    }
    
    .status-dot.invoice-partial {
        background: #f59e0b;
    }
    
    .status-dot.invoice-pending {
        background: #94a3b8;
    }
    
    .status-dot.invoice-overdue {
        background: #ef4444;
    }
    
    .status-select.invoice-paid {
        color: #10b981;
        border-color: #86efac;
        background-color: #f0fdf4;
    }
    
    .status-select.invoice-partial {
        color: #f59e0b;
        border-color: #fcd34d;
        background-color: #fffbeb;
    }
    
    .status-select.invoice-pending {
        color: #64748b;
        border-color: #e2e8f0;
        background-color: #f8fafc;
    }
    
    .status-select.invoice-overdue {
        color: #ef4444;
        border-color: #fecaca;
...
document.querySelector('.btn-show-total')?.addEventListener('click', function() {
    ...
    document.querySelectorAll('.data-row').forEach(row => {
        if (row.style.display !== 'none') {
            const amountText = row.querySelector('.col-amount')?.textContent.trim().split('\n')[0] || '0';
